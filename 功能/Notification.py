...
class NotificationIcon:
    ...
    Types = {Info: None, Success: None, Warning: None, Error: None, Close: None}
...
class NotificationItem(QWidget):
    ...
    def __init__(self, title, message, item, *args, ntype=0, callback=None, **kwargs):
        ...
        layout.addItem(
            QSpacerItem(40, 20, QSizePolicy.Expanding, QSizePolicy.Minimum), 0, 2
        )
...
class NotificationWindow(QListWidget):
    ...
    def error(cls, title, message, callback=None):
        ...
        item = QListWidgetItem(cls._instance)
        w = NotificationItem(
            title, message, item, ntype=NotificationIcon.Error, callback=callback
        )
        w.closed.connect(cls._instance.removeItem)
...
if __name__ == "__main__":
    ...
    layout.addWidget(
        QPushButton(
            ...
            clicked=lambda: NotificationWindow.info(
                "提示", "这是一条会自动关闭的消息", callback=callback
            ),
        )
    )
    layout.addWidget(
    ...
    layout.addWidget(
        QPushButton(
            "Warning",
            w,
            clicked=lambda: NotificationWindow.warning(
                "提示", "这是提示文案这是提示文案这是提示文案这是提示文案。", callback=callback
            ),
        )
    )
    layout.addWidget(
        QPushButton(
            "Error",
            w,
            clicked=lambda: NotificationWindow.error(
                "提示",
